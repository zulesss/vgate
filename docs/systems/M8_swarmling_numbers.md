...
| **move_speed** | 8.0 u/s | Parity с player walk ровно при cap=100 (player walk = 8.0 при максимальном cap). Hook-усиление: cap 100 = swarmling равный, любой drop ниже cap = swarm тянет (ratio_diff = (100−cap)/100 × 8.0). Was 7.7 (safe-zone на cap > 96) — playtest 2026-05-01 показал что safe-зона слишком широкая, игрок просто бежит при любом decent cap. Buff +0.3 делает hook ощутимым: outrun возможен только с перфектным cap=100. Аналог: DOOM 2016 Imp (быстрый, но не hitscan-стремительный) |
